...
public class ValidationItemControllerV2 {
    ...
    private final ItemValidator itemValidator;//@Componet로 등록을 해뒀기 때문에 자동으로 DI 시행
    ...
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes
        ...
        if(item.getQuantity() == null || item.getQuantity() >= 9999){
            bindingResult.addError(new FieldError("item", "quantity", "수량은 9,999 이하여야 합니다.."));
        }
    ...
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes
        ...
        if(item.getPrice() == null || item.getPrice() < 1000 ||
            ...
            bindingResult.addError(new FieldError("item", "price", item.getPrice(),
                    false,null,null,"가격은 1,000 ~ 1,000,000" +
                    "사이어야 합니다."));
        }

        if(item.getQuantity() == null || item.getQuantity() >= 9999){
            bindingResult.addError(new FieldError("item", "quantity", item.getQuantity(),
                    false,null,null,"수량은 9,999 이하여야 합니다.."));
        }
...
//            model.addAttribute("errors", bindingResult);
    ...
    public String addItemV4(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes
                          , Model model) {
//      아래의 if 문을 한줄로 줄일 수 있으나, Empty, 공백 같은 단순한 조건만 검사 가능
//        ValidationUtils.rejectIfEmptyOrWhitespace(bindingResult, "itemName", "required");
        //검증 로직

        if (!StringUtils.hasText(item.getItemName())){
            bindingResult.rejectValue("itemName", "required"); // errorCode에 input + obj Name + field Name이 됨
        }
        if(item.getPrice() == null || item.getPrice() < 1000 ||
                item. getPrice() > 1000000){
